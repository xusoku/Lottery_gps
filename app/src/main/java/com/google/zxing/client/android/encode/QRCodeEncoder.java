...
public final class QRCodeEncoder {
    ...
    private static final int WHITE = 0xFFFFFFFF;//0xFFFFFFFF
    ...
    public QRCodeEncoder(Context activity, Intent intent, int dimension, boolean useVCard,ErrorCorrectionLevel errorCorrectionLevel) throws WriterException {
     ...
     */
    public Bitmap encodeAsBitmap(int customColor, int margin) throws WriterException {
        ...
        //1.1去白边
        int[] rec = result.getEnclosingRectangle();
        //1.2加白边
        ...
        for (int i = margin; i < resWidth - margin; i++) {
            for (int j = margin; j < resHeight - margin; j++) {
                if (result.get(i + rec[0] - margin, j + rec[1] - margin)) {
                    resMatrix.set(i, j);
                }
            }
        }
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                pixels[offset + x] = resMatrix.get(x, y) ? BLACK : customColor;
            }
        }
